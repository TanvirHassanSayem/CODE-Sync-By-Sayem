...
const RunCodeContextProvider = ({ children }: { children: ReactNode }) => {
    ...
    try {
        ...
        // Check for valid language and version before making the request
        if (!language || !version) {
            toast.error("Invalid language or version selected");
            setIsRunning(false);
            return;
        }
        ...
        setIsRunning(false);
        toast.dismiss();
        toast.error("Failed to run the code. Please try again later.");
    }
};
